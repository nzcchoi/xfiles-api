...
namespace Global.Storage
{
    public class BlobService
    {
        ...
        public async Task<bool> UploadFileAsync(string name, string filePath, byte[] fileStream)
        {
            ...
            if (CloudStorageAccount.TryParse(_storageConnectionString, out CloudStorageAccount storageAccount))
            {
                ...
                cloudBlobContainer = cloudBlobClient.GetContainerReference(name);
                CloudBlockBlob cloudBlockBlob = cloudBlobContainer.GetBlockBlobReference(filePath);
                await cloudBlockBlob.UploadFromByteArrayAsync(fileStream, 0, fileStream.Length);
        ...
        public async Task<byte[]> DownloadFileAsync(string name, string filePath)
        {
            CloudBlobContainer cloudBlobContainer = null;
            // Check whether the connection string can be parsed.
            if (CloudStorageAccount.TryParse(_storageConnectionString, out CloudStorageAccount storageAccount))
        ...
        public async Task<bool> FileExistsAsync(string name, string filePath)
        {
            CloudBlobContainer cloudBlobContainer = null;
            // Check whether the connection string can be parsed.
            if (CloudStorageAccount.TryParse(_storageConnectionString, out CloudStorageAccount storageAccount))
